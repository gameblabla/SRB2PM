...
#include "IMG_xpm.h" //Alam: I don't want to add SDL_Image.dll/so
...
#define SRB_KEY_UP SDLK_u
#define SRB_KEY_LEFT SDLK_l
#define SRB_KEY_RIGHT SDLK_r
#define SRB_KEY_DOWN SDLK_d

#define SRB_KEY_LCTRL SDLK_a
#define SRB_KEY_LALT SDLK_b
#define SRB_KEY_LSHIFT SDLK_x
#define SRB_KEY_SPACE SDLK_y

#define SRB_KEY_TAB SDLK_m
#define SRB_KEY_BACKSPACE SDLK_n

#define SRB_KEY_RETURN SDLK_s
#define SRB_KEY_ESCAPE SDLK_k

#define SRB_KEY_HOME SDLK_q

#else

#define SRB_KEY_UP SDLK_UP
#define SRB_KEY_LEFT SDLK_LEFT
#define SRB_KEY_RIGHT SDLK_RIGHT
#define SRB_KEY_DOWN SDLK_DOWN

#define SRB_KEY_LCTRL SDLK_LCTRL
#define SRB_KEY_LALT SDLK_LALT
#define SRB_KEY_LSHIFT SDLK_LSHIFT
#define SRB_KEY_SPACE SDLK_SPACE

#define SRB_KEY_TAB SDLK_TAB
#define SRB_KEY_BACKSPACE SDLK_BACKSPACE

#define SRB_KEY_RETURN SDLK_RETURN
#define SRB_KEY_ESCAPE SDLK_ESCAPE

#define SRB_KEY_HOME SDLK_HOME
...
rendermode_t chosenrendermode = render_soft; // set by command line arguments
...
/* The new OD firmwares support 8-bits paletted support */
	...
	s_width = videoInfo->current_w;
	s_height = videoInfo->current_h;
	
	switch(s_width)
	{
		case 1280:
			printf("\nDevice : HDMI output\n");
			vidSurface = SDL_SetVideoMode(640, 360, 16, SDL_HWSURFACE | SDL_HWPALETTE);
		break;
		case 640:
			printf("\nDevice : RG-350M/RG-300X\n");
			vidSurface = SDL_SetVideoMode(640, 480, 8, SDL_HWSURFACE | SDL_HWPALETTE);
		break;
		case 320:
			if (s_height == 240)
			{
				printf("\nDevice : RG-350 / RG-350P\n");
				vidSurface = SDL_SetVideoMode(320, 240, 8, SDL_HWSURFACE | SDL_HWPALETTE);
			}
			else
			{
				printf("\nDevice : RG-280V/RG-280M\n");
	...
	width = 320;
	height = 240;
	vidSurface = SDL_SetVideoMode(width, height, 16, SDL_HWSURFACE);
	if (!vidSurface)
		return;
#else
...
#define GP2X_BUTTON_VOLDOWN         (17)
	if ((event->type == ev_keydown || event->type == ev_keyup) && (KEY_JOY1 <= event->key && event->key <= KEY_JOY1+JOYBUTTONS))
	{
		INT32 button = event->key-KEY_JOY1;
		if (button <= 7)
		{
			static UINT8 DPAD = 0;
		...
		else switch (button)
		{
			...
			case GP2X_BUTTON_VOLDOWN:
				event->key = KEY_JOY1+7;
				break;
			default:
				break;
		}
		//I_OutputMsg("Button %i: event key %i and type: %i\n", button, event->key, event->type);
	}
#elif defined(_PSP)
	if (event->key > KEY_JOY1 + 9 + 2) // All button after D-Pad and Select/Start
		event->key -= 4; // remap D-pad to Hats, offset of -4
	else if (event->key == KEY_JOY1 + 6) // Down
		event->key = KEY_HAT1+1;
	else if (event->key == KEY_JOY1 + 7) // Left
		event->key = KEY_HAT1+2;
	else if (event->key == KEY_JOY1 + 8) // Up
		event->key = KEY_HAT1+0;
	else if (event->key == KEY_JOY1 + 9) // Right
		event->key = KEY_HAT1+3;
	else if (event->key == KEY_JOY1 + 10) // Select
		event->key = KEY_TAB;
	else if (event->key == KEY_JOY1 + 11) // Start
		event->key = KEY_ESCAPE;
#else
	(void)event;
#endif
...
void I_FinishUpdate(void)
{
	...
	if (vidSurface->format->BitsPerPixel == 8)
	{
		...
		memcpy(vidSurface->pixels, vid.buffer, vid.width * vid.height);
